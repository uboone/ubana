                        ...
                        std::vector<float> &pitches)
{
  ...
  int vermode = 0;
    ...
    {
       if(vermode != 0)std::cout << "[EnergyHelper} loop over clusters" << std::endl;
        ...
        if (pitch >= 0)
        {
          if(vermode != 0) std::cout << "[EnergyHelper} pitch > 0" << std::endl;
        ...
        if (cluster_length <= 0)
            continue;
        if(vermode != 0) std::cout << "[EnergyHelper} clusterlength > 0 " << std::endl;
      ...
      if(vermode == 10){
        for (auto &hit : hits)
        {
          ...
          if(vermode != 0) std::cout << "[EnergyHelper} loop over hits, got hitpos x" << std::endl;
            ...
            if (is_within)
            {
              if(vermode != 0) std::cout << "[EnergyHelper} loop over hits, is within" << std::endl;
        ...
        if (dqdxs.size() > 0)
        {
          if(vermode != 0) std::cout << "[EnergyHelper} dqdxs > 0" << std::endl;
   ...
   if(vermode != 0) std::cout << "[EnergyHelper} End of dQdx function" << std::endl;
}
