...
void ExploreTruth::analyze(art::Event const & evt)
{
  ...
  art::InputTag partTag {std::string ("largeant")};
  art::ValidHandle< std::vector<simb::MCParticle> > const & partHandle = evt.getValidHandle< std::vector<simb::MCParticle> >(partTag);
  art::InputTag truthTag {std::string ("generator")};
  art::ValidHandle< std::vector<simb::MCTruth> > const & truthHandle = evt.getValidHandle< std::vector<simb::MCTruth> >(truthTag);

  art::Ptr<simb::MCTruth> truth(truthHandle,0);
  for(int i=0; i!=truth->NParticles(); i++)
  {
    // simb::MCParticle const & p1 = truth->GetParticle(i);
    // std::string spaces("");
    // std::string type("TRUTH");
    // printf("%s|_%s %i | PDG: %i, Process: %s\n", spaces.c_str(), type.c_str(), p1.TrackId(), p1.PdgCode(), p1.Process().c_str());
    // printf("%s  |_End process: %s\n", spaces.c_str(), p1.EndProcess().c_str());
    // printf("%s  |_Number of daughters: %i\n", spaces.c_str(), p1.NumberDaughters());
    // printf("%s  |_Status code: %i\n",spaces.c_str(), p1.StatusCode());
    // printf("%s  |_Start coordinates: [ %.1f, %.1f, %.1f, %.1f ]\n", spaces.c_str(), p1.Vx(), p1.Vy(), p1.Vz(), p1.T());
    // printf("%s  |_End coordinates: [ %.1f, %.1f, %.1f, %.1f ]\n", spaces.c_str(), p1.EndX(), p1.EndY(), p1.EndZ(), p1.EndT());
    // printf("%s  |_Start/End energies: [ %.3f, %.3f ]\n", spaces.c_str(), p1.E(), p1.EndE());
  }
  ...
  // art::InputTag rawTag {std::string ("daq")};
  // art::ValidHandle< std::vector<raw::RawDigit> > const & rawHandle = evt.getValidHandle< std::vector<raw::RawDigit> >(rawTag);
  // printf("L: %i\n", int((*rawHandle).size()));

  art::InputTag rawTag {std::string ("caldata")};
  art::ValidHandle< std::vector<raw::SparseRawDigit> > const & rawHandle = evt.getValidHandle< std::vector<raw::SparseRawDigit> >(rawTag);
  printf("L: %i\n", int((*rawHandle).size()));
  for(int i=0; i!=20; i++)
  {
    art::Ptr<raw::SparseRawDigit> srd(rawHandle,i);
    printf("RDS: %i\n", i);
    printf("|_Pedestal: %.5f\n|_Width: %.5f\n", srd->GetPedestal(), srd->GetSigma());
  }
