...
void SingleParticleMomentumStudy::beginJob()
{
  ...
  tDataTree->Branch("endProcess",&endProcess);
        ...
        std::map< int,art::Ptr<simb::MCParticle> > mcpMap)
{
  ...
  for(int j=0; j!=mcp->NumberDaughters(); j++)
  {
    ...
    {
      if (daughter->Process() == std::string("Decay")) endProcess = std::string("Decay");
      if (daughter->Process() == std::string("pi+Inelastic")) endProcess = std::string("pi+Inelastic");
      if (daughter->Process() == std::string("pi-Inelastic")) endProcess = std::string("pi-Inelastic"); 
    ...
    {
      if (daughter->Process() == std::string("Decay")) endProcess = std::string("Decay");
      if (daughter->Process() == std::string("muMinusCaptureAtRest")) endProcess = std::string("muMinusCaptureAtRest");
      if (daughter->Process() == std::string("muPlusCaptureAtRest")) endProcess = std::string("muPlusCaptureAtRest");
...
void SingleParticleMomentumStudy::analyze(art::Event const & evt)
{
  ...
  float mass_ratio = pion_mass / muon_mass;
  trkf::TrackMomentumCalculator tmc;
  rangeTruth_muon_momentum = tmc.GetTrackMomentum(truth_length,13);
  rangeTruth_pion_momentum = tmc.GetTrackMomentum(truth_length,13) * mass_ratio;
  range_muon_momentum = tmc.GetTrackMomentum(reco_length,13);
  range_pion_momentum = tmc.GetTrackMomentum(reco_length,13) * mass_ratio;
  // MCS momentum
