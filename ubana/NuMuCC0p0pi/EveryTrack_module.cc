...
private:
  ...
  std::vector<double> trk_crt_time_T0corr;// The CRT time of the hit which matched to the track
...
void EveryTrack::analyze(art::Event const& evt)
{
  ...
  for(unsigned int i_trk = 0; i_trk < daughter_Tracks.size(); i_trk++){
    ...
    trk_vtx_x.push_back(Trk_vtx_SCEcorr.X());
    trk_vtx_y.push_back(Trk_vtx_SCEcorr.Y());
    trk_vtx_z.push_back(Trk_vtx_SCEcorr.Z());

    trk_vtx_x_noSCE.push_back(Trk_vtx.X());
    ...
    // dEdx_half
    if(half_size_pl0 != 0){
      dEdx_pl0_start_half.push_back(std::accumulate(dEdx_pl0.end() - half_size_pl0, dEdx_pl0.end(), 0.) / half_size_pl0);
      dEdx_pl0_end_half.push_back(std::accumulate(dEdx_pl0.begin(), dEdx_pl0.begin() + half_size_pl0, 0. ) / half_size_pl0);
    }
    else{
      dEdx_pl0_start_half.push_back(0);
      dEdx_pl0_end_half.push_back(0);
    }
     
    if(half_size_pl1 != 0){
      dEdx_pl1_start_half.push_back(std::accumulate(dEdx_pl1.end() - half_size_pl1, dEdx_pl1.end(), 0.) / half_size_pl1);
      dEdx_pl1_end_half.push_back(std::accumulate(dEdx_pl1.begin(), dEdx_pl1.begin() + half_size_pl1, 0. ) / half_size_pl1);
    }
    else{
      dEdx_pl1_start_half.push_back(0);
      dEdx_pl1_end_half.push_back(0);
    }

    if(half_size_pl2 != 0){
      dEdx_pl2_start_half.push_back(std::accumulate(dEdx_pl2.end() - half_size_pl2, dEdx_pl2.end(), 0.) / half_size_pl2);
      dEdx_pl2_end_half.push_back(std::accumulate(dEdx_pl2.begin(), dEdx_pl2.begin() + half_size_pl2, 0. ) / half_size_pl2);
    }
    else{
      dEdx_pl2_start_half.push_back(0);
      dEdx_pl2_end_half.push_back(0);
    }
    // dEdx_1020
    if (dEdx_pl0.size()<=40) {
      ...
      dEdx_pl0_start1020.push_back(std::accumulate(dEdx_pl0.end() - 20, dEdx_pl0.end() - 10, 0.) / 10.);
      dEdx_pl0_end1020.push_back(std::accumulate(dEdx_pl0.begin() + 10, dEdx_pl0.begin() + 20, 0.) / 10.);
    }
    if (dEdx_pl1.size()<=40) {
      ...
      dEdx_pl1_start1020.push_back(std::accumulate(dEdx_pl1.end() - 20, dEdx_pl1.end() - 10, 0.) / 10.);
      dEdx_pl1_end1020.push_back(std::accumulate(dEdx_pl1.begin() + 10, dEdx_pl1.begin() + 20, 0.) / 10.);
    }
    if (dEdx_pl2.size()<=40) {
      ...
      dEdx_pl2_start1020.push_back(std::accumulate(dEdx_pl2.end() - 20, dEdx_pl2.end() - 10, 0.) / 10.);
      dEdx_pl2_end1020.push_back(std::accumulate(dEdx_pl2.begin() + 10, dEdx_pl2.begin() + 20, 0.) / 10.);
    }

    dEdx_pl0_1020_ratio.push_back(dEdx_pl0_end1020.back() / (dEdx_pl0_end1020.back() + dEdx_pl0_start1020.back()));
    dEdx_pl0_half_ratio.push_back(dEdx_pl0_end_half.back() / (dEdx_pl0_end_half.back() + dEdx_pl0_start_half.back()));

    dEdx_pl1_1020_ratio.push_back(dEdx_pl1_end1020.back() / (dEdx_pl1_end1020.back() + dEdx_pl1_start1020.back()));
    dEdx_pl1_half_ratio.push_back(dEdx_pl1_end_half.back() / (dEdx_pl1_end_half.back() + dEdx_pl1_start_half.back()));

    dEdx_pl2_1020_ratio.push_back(dEdx_pl2_end1020.back() / (dEdx_pl2_end1020.back() + dEdx_pl2_start1020.back()));
    dEdx_pl2_half_ratio.push_back(dEdx_pl2_end_half.back() / (dEdx_pl2_end_half.back() + dEdx_pl2_start_half.back()));
    ...
    if(nr_pl0_mid > 0){ //pl0
      dEdx_pl0_mid.push_back(std::accumulate(dEdx_pl0.begin() + nr_pl0_mid , dEdx_pl0.begin() + 2*nr_pl0_mid, 0.) / nr_pl0_mid);
    }
    else {
      dEdx_pl0_mid.push_back(0);
    }
    if(nr_pl1_mid > 0){ //pl1
      dEdx_pl1_mid.push_back(std::accumulate(dEdx_pl1.begin() + nr_pl1_mid , dEdx_pl1.begin() + 2*nr_pl1_mid, 0.) / nr_pl1_mid);
    }
    else {
      dEdx_pl1_mid.push_back(0);
    }
    if(nr_pl2_mid > 0){ //pl2
      dEdx_pl2_mid.push_back(std::accumulate(dEdx_pl2.begin() + nr_pl2_mid , dEdx_pl2.begin() + 2*nr_pl2_mid, 0.) / nr_pl2_mid);
    }
    else {
      dEdx_pl2_mid.push_back(0);
    }
    ...
    // - median dE/dx
    // pl 0
    if(copy_dEdx_pl0.size() != 0){
      if(fmod(copy_dEdx_pl0.size() / 2., 1) == 0){
        dEdx_median_pl0.push_back((copy_dEdx_pl0[copy_dEdx_pl0.size() / 2 - 1] + copy_dEdx_pl0[copy_dEdx_pl0.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl0.size() / 2., 1) == 0.5){
        dEdx_median_pl0.push_back(copy_dEdx_pl0[copy_dEdx_pl0.size() / 2]);
      }
    }
    else{
      dEdx_median_pl0.push_back(0);
    }
    // pl 1
    if(copy_dEdx_pl1.size() != 0){
      if(fmod(copy_dEdx_pl1.size() / 2., 1) == 0){
        dEdx_median_pl1.push_back((copy_dEdx_pl1[copy_dEdx_pl1.size() / 2 - 1] + copy_dEdx_pl1[copy_dEdx_pl1.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl1.size() / 2., 1) == 0.5){
        dEdx_median_pl1.push_back(copy_dEdx_pl1[copy_dEdx_pl1.size() / 2]);
      }
    }
    else{
      dEdx_median_pl1.push_back(0);
    }
    // pl 2
    if(copy_dEdx_pl2.size() != 0){
      if(fmod(copy_dEdx_pl2.size() / 2., 1) == 0){
        dEdx_median_pl2.push_back((copy_dEdx_pl2[copy_dEdx_pl2.size() / 2 - 1] + copy_dEdx_pl2[copy_dEdx_pl2.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl2.size() / 2., 1) == 0.5){
        dEdx_median_pl2.push_back(copy_dEdx_pl2[copy_dEdx_pl2.size() / 2]);
      }
    }
    else{
      dEdx_median_pl2.push_back(0);
    }
    ...
    if(half_size_pl0_clean != 0){
      dEdx_pl0_start_half_clean.push_back(std::accumulate(dEdx_pl0_clean.end() - half_size_pl0_clean, dEdx_pl0_clean.end(), 0.) / half_size_pl0_clean);
      dEdx_pl0_end_half_clean.push_back(std::accumulate(dEdx_pl0_clean.begin(), dEdx_pl0_clean.begin() + half_size_pl0_clean, 0. ) / half_size_pl0_clean);
    }
    else{
      dEdx_pl0_start_half_clean.push_back(0);
      dEdx_pl0_end_half_clean.push_back(0);
    }
    if(half_size_pl1_clean != 0){
      dEdx_pl1_start_half_clean.push_back(std::accumulate(dEdx_pl1_clean.end() - half_size_pl1_clean, dEdx_pl1_clean.end(), 0.) / half_size_pl1_clean);
      dEdx_pl1_end_half_clean.push_back(std::accumulate(dEdx_pl1_clean.begin(), dEdx_pl1_clean.begin() + half_size_pl1_clean, 0. ) / half_size_pl1_clean);
    }
    else{
      dEdx_pl1_start_half_clean.push_back(0);
      dEdx_pl1_end_half_clean.push_back(0);
    }
    if(half_size_pl2_clean != 0){
      dEdx_pl2_start_half_clean.push_back(std::accumulate(dEdx_pl2_clean.end() - half_size_pl2_clean, dEdx_pl2_clean.end(), 0.) / half_size_pl2_clean);
      dEdx_pl2_end_half_clean.push_back(std::accumulate(dEdx_pl2_clean.begin(), dEdx_pl2_clean.begin() + half_size_pl2_clean, 0. ) / half_size_pl2_clean);
    }
    else{
      dEdx_pl2_start_half_clean.push_back(0);
      dEdx_pl2_end_half_clean.push_back(0);
    }
    // dEdx_1020
    if (dEdx_pl0_clean.size()<=40) {
      dEdx_pl0_start1020_clean.push_back(dEdx_pl0_start_half_clean.back());
      dEdx_pl0_end1020_clean.push_back(dEdx_pl0_end_half_clean.back());
    }
    else{
      dEdx_pl0_start1020_clean.push_back(std::accumulate(dEdx_pl0_clean.end() - 20, dEdx_pl0_clean.end() - 10, 0.) / 10.);
      dEdx_pl0_end1020_clean.push_back(std::accumulate(dEdx_pl0_clean.begin() + 10, dEdx_pl0_clean.begin() + 20, 0.) / 10.);
    }
    if (dEdx_pl1_clean.size()<=40) {
      dEdx_pl1_start1020_clean.push_back(dEdx_pl1_start_half_clean.back());
      dEdx_pl1_end1020_clean.push_back(dEdx_pl1_end_half_clean.back());
    }
    else{
      dEdx_pl1_start1020_clean.push_back(std::accumulate(dEdx_pl1_clean.end() - 20, dEdx_pl1_clean.end() - 10, 0.) / 10.);
      dEdx_pl1_end1020_clean.push_back(std::accumulate(dEdx_pl1_clean.begin() + 10, dEdx_pl1_clean.begin() + 20, 0.) / 10.);
    }
    if (dEdx_pl2_clean.size()<=40) {
      dEdx_pl2_start1020_clean.push_back(dEdx_pl2_start_half_clean.back());
      dEdx_pl2_end1020_clean.push_back(dEdx_pl2_end_half_clean.back());
    }
    else{
      dEdx_pl2_start1020_clean.push_back(std::accumulate(dEdx_pl2_clean.end() - 20, dEdx_pl2_clean.end() - 10, 0.) / 10.);
      dEdx_pl2_end1020_clean.push_back(std::accumulate(dEdx_pl2_clean.begin() + 10, dEdx_pl2_clean.begin() + 20, 0.) / 10.);
    }

    dEdx_pl0_1020_ratio_clean.push_back(dEdx_pl0_end1020_clean.back() / (dEdx_pl0_end1020_clean.back() + dEdx_pl0_start1020_clean.back()));
    dEdx_pl0_half_ratio_clean.push_back(dEdx_pl0_end_half_clean.back() / (dEdx_pl0_end_half_clean.back() + dEdx_pl0_start_half_clean.back()));

    dEdx_pl1_1020_ratio_clean.push_back(dEdx_pl1_end1020_clean.back() / (dEdx_pl1_end1020_clean.back() + dEdx_pl1_start1020_clean.back()));
    dEdx_pl1_half_ratio_clean.push_back(dEdx_pl1_end_half_clean.back() / (dEdx_pl1_end_half_clean.back() + dEdx_pl1_start_half_clean.back()));

    dEdx_pl2_1020_ratio_clean.push_back(dEdx_pl2_end1020_clean.back() / (dEdx_pl2_end1020_clean.back() + dEdx_pl2_start1020_clean.back()));
    dEdx_pl2_half_ratio_clean.push_back(dEdx_pl2_end_half_clean.back() / (dEdx_pl2_end_half_clean.back() + dEdx_pl2_start_half_clean.back()));
    ...
    if(nr_pl0_mid_clean > 0){ //pl0
      dEdx_pl0_mid_clean.push_back(std::accumulate(dEdx_pl0_clean.begin() + nr_pl0_mid_clean , dEdx_pl0_clean.begin() + 2*nr_pl0_mid_clean, 0.) / nr_pl0_mid_clean);
    }
    else {
      dEdx_pl0_mid_clean.push_back(0);
    }
    if(nr_pl1_mid_clean > 0){ //pl1
      dEdx_pl1_mid_clean.push_back(std::accumulate(dEdx_pl1_clean.begin() + nr_pl1_mid_clean , dEdx_pl1_clean.begin() + 2*nr_pl1_mid_clean, 0.) / nr_pl1_mid_clean);
    }
    else {
      dEdx_pl1_mid_clean.push_back(0);
    }
    if(nr_pl2_mid_clean > 0){ //pl2
      dEdx_pl2_mid_clean.push_back(std::accumulate(dEdx_pl2_clean.begin() + nr_pl2_mid_clean , dEdx_pl2_clean.begin() + 2*nr_pl2_mid_clean, 0.) / nr_pl2_mid_clean);
    }
    else {
      dEdx_pl2_mid_clean.push_back(0);
    }
    ...
    // - median dE/dx
    // pl 0
    if(copy_dEdx_pl0_clean.size() != 0){
      if(fmod(copy_dEdx_pl0_clean.size() / 2., 1) == 0){
        dEdx_median_pl0_clean.push_back((copy_dEdx_pl0_clean[copy_dEdx_pl0_clean.size() / 2 - 1] + copy_dEdx_pl0_clean[copy_dEdx_pl0_clean.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl0_clean.size() / 2., 1) == 0.5){
        dEdx_median_pl0_clean.push_back(copy_dEdx_pl0_clean[copy_dEdx_pl0_clean.size() / 2]);
      }
    }
    else{
      dEdx_median_pl0_clean.push_back(0);
    }
    // pl 1
    if(copy_dEdx_pl1_clean.size() != 0){
      if(fmod(copy_dEdx_pl1_clean.size() / 2., 1) == 0){
        dEdx_median_pl1_clean.push_back((copy_dEdx_pl1_clean[copy_dEdx_pl1_clean.size() / 2 - 1] + copy_dEdx_pl1_clean[copy_dEdx_pl1_clean.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl1_clean.size() / 2., 1) == 0.5){
        dEdx_median_pl1_clean.push_back(copy_dEdx_pl1_clean[copy_dEdx_pl1_clean.size() / 2]);
      }
    }
    else{
      dEdx_median_pl1_clean.push_back(0);
    }
    // pl 2
    if(copy_dEdx_pl2_clean.size() != 0){
      if(fmod(copy_dEdx_pl2_clean.size() / 2., 1) == 0){
        dEdx_median_pl2_clean.push_back((copy_dEdx_pl2_clean[copy_dEdx_pl2_clean.size() / 2 - 1] + copy_dEdx_pl2_clean[copy_dEdx_pl2_clean.size() / 2]) / 2.);
      }
      else if (fmod(copy_dEdx_pl2_clean.size() / 2., 1) == 0.5){
        dEdx_median_pl2_clean.push_back(copy_dEdx_pl2_clean[copy_dEdx_pl2_clean.size() / 2]);
      }
    }
    else{
      dEdx_median_pl2_clean.push_back(0);
    }
    //-- PID
     PID pid;
  ...
  dEdx_pl0_clean.clear();
  dEdx_pl1_clean.clear();
  dEdx_pl2_clean.clear();

  copy_dEdx_pl0_clean.clear();
  copy_dEdx_pl1_clean.clear();
  copy_dEdx_pl2_clean.clear();
  } // End of track loop 
  ...
  trk_crt_time_T0corr.clear();// The CRT time of the hit which matched to the track
  ...
  dEdx_pl0_1020_ratio.clear();
  dEdx_pl0_half_ratio.clear();
  dEdx_pl1_1020_ratio.clear();
  dEdx_pl1_half_ratio.clear();
  dEdx_pl2_1020_ratio.clear();
  dEdx_pl2_half_ratio.clear();

  dEdx_pl0_mid.clear();
  dEdx_pl1_mid.clear();
  dEdx_pl2_mid.clear();

  dEdx_median_pl0.clear();
  dEdx_median_pl1.clear();
  dEdx_median_pl2.clear();

  dEdx_pl0_start_half_clean.clear();
  dEdx_pl1_start_half_clean.clear();
  dEdx_pl2_start_half_clean.clear();
  dEdx_pl0_end_half_clean.clear();
  dEdx_pl1_end_half_clean.clear();
  dEdx_pl2_end_half_clean.clear();

  dEdx_pl0_start1020_clean.clear();
  dEdx_pl1_start1020_clean.clear();
  dEdx_pl2_start1020_clean.clear();
  dEdx_pl0_end1020_clean.clear();
  dEdx_pl1_end1020_clean.clear();
  dEdx_pl2_end1020_clean.clear();

  dEdx_pl0_1020_ratio_clean.clear();
  dEdx_pl0_half_ratio_clean.clear();
  dEdx_pl1_1020_ratio_clean.clear();
  dEdx_pl1_half_ratio_clean.clear();
  dEdx_pl2_1020_ratio_clean.clear();
  dEdx_pl2_half_ratio_clean.clear();

  dEdx_pl0_mid_clean.clear();
  dEdx_pl1_mid_clean.clear();
  dEdx_pl2_mid_clean.clear();

  dEdx_median_pl0_clean.clear();
  dEdx_median_pl1_clean.clear();
  dEdx_median_pl2_clean.clear();
