  ...
  //-------True
    ...
    if (if_cosmic){
      ...
      for (unsigned int i_mc = 0; i_mc < MCTruthCollection.size(); i_mc++){
        //std::vector<simb::MCParticle*> assoMC = MCtToMCpAsso.at(MCTruthCollection[i_mc].key());
        ...
        for (unsigned int i_mcp = 0; i_mcp < assoMC.size(); i_mcp++){
          //std::cout<<"assoMC[i_mcp]: "<< assoMC[i_mcp]->TrackId()<<std::endl;
          //std::cout<<"selected_MCparticle: "<< selected_MCparticle.get()->TrackId()<<std::endl;
          if (assoMC[i_mcp] == selected_MCparticle.get()){
