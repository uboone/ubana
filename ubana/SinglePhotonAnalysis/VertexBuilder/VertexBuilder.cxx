...
void VertexBuilder::AssociateTracks(ParticleAssociations & pas) {//Group tracks
  ...
  std::multimap<size_t, geoalgo::Point_t const *> pn;//CHECK prepare to store track start/end points

  for(size_t const i : fdetos->GetTrackIndices()) {//load up tracks into pn.
  ...
  if(fverbose) {
	...
	if(pn.size()) {
		...
		for(auto p : pn) {
			...
			std::cout << fdetos->GetTrack(p.first).ftrajectory.Length();//THIS GIVE VERTEX COORDINATES, ftrajectory is the obejct in art library
			cout<<"\n\nLine 88\n\n"<<endl;
			std::cout << fdetos->GetTrack(p.first).ftrajectory.Length();//THIS GIVE VERTEX COORDINATES
			last_id = p.first;
		}
		std::cout << "\n";
	}
